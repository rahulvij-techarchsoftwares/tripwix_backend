!function () {
    ...
    jQuery(function($){
    	...
    	var Featured = {
			...
			widget: $("#property-featured-widget"),
			clear_pub_date: $('.featured-widget-clear_pub_date'),
			clear_pub_end_date: $('.featured-widget-clear_pub_end_date'),
			init: function(options){
				options = $.extend({
					'active': 'is_featured',
					'pub_date': 'featured_date',
					'pub_end_date': 'featured_end_date'
				}, options);
				...
				if(this.pub_date_field.length > 0){
					...
					this.widget.find(".featured-widget-pub_date").remove();
				}
				if(this.pub_end_date_field.length > 0){
					...
					this.widget.find(".featured-widget-pub_end_date").remove();
				}
				...
				if(this.pub_date_field.length > 0){
					if(this.pub_date_field.hasClass("errors") || this.pub_date_field.hasClass("has-error")){
						this.widget.find(".featured-widget-pub_date").click();
					}
				}
				if(this.pub_end_date_field.length > 0){
					if(this.pub_end_date_field.hasClass("errors") || this.pub_end_date_field.hasClass("has-error")){
						this.widget.find(".featured-widget-pub_end_date").click();
					}
			...
			onPubDate: function(){
				...
				this.widget.find(".featured-widget-visible .info span").text(string_date);
				this.widget.find(".featured-widget-hidden .info span").text(string_date);

				if(!this.is_active()){
					this.widget.find(".featured-widget-hidden :input").prop("checked", true);
					return false;
				}
				...
				if(this.toDate(string_date) > new Date()){
					//FUTURE
					this.widget.find(".featured-widget-hidden :input").prop("checked", true);
					this.widget.find(".featured-widget-visible .info").hide();
					this.widget.find(".featured-widget-hidden .info").show();

				} else {
					//PAST
					this.widget.find(".featured-widget-visible :input").prop("checked", true);
					this.widget.find(".featured-widget-visible .info").show();
					this.widget.find(".featured-widget-hidden .info").hide();
				}
			},
			open_pub_date: function(){
				this.active_field.find(":input").eq(0).prop("checked", true);
				this.widget.find(".featured-widget-pub_date").hide();
				if(this.widget.find(".featured-widget-hidden :input").is(":checked")){
					this.pub_date_field.find(":input[type!='hidden']").eq(0).val("");
				}
				this.pub_date_field.show();
			},
			close_pub_date: function(){
				this.widget.find(".featured-widget-pub_date").show();
				this.pub_date_field.hide();
			},
			clear_date: function(){
			...
			events: function(){
				...
				this.widget.find(".featured-widget-pub_date").on("click", function(){
					self.open_pub_date();
					return false;
				});

				this.widget.find(".featured-widget-pub_end_date").on("click", function(){
