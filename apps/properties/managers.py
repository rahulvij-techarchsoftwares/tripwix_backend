...
class PropertyQuerysetManager(models.Manager):
    def published(self):
        return self.get_queryset().filter(
            Q(is_active=True)
            | Q(
                Q(publication_date__lte=timezone.now())
                & Q(Q(publication_date__isnull=True) | Q(publication_end_date__gt=timezone.now()))
            )
        )


class PropertyTypeManager(models.Manager):
